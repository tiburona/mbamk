...
processes = {
    ...
    'flask': {
        'cmd': {
            'local': ['flask run'],
            'trusted': ['flask run'],
            'docker': ['flask db upgrade', "gunicorn -w 1 -b :8000 --worker-class gevent 'cookiecutter_mbam.app:create_app()'"],
            'staging': ['flask db upgrade', "gunicorn -w 1 -b :8000 --worker-class gevent 'cookiecutter_mbam.app:create_app()'"]
        } ,
        'label': ('FLASK', 'BLUE')
    }
}
